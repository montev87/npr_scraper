...
# --- Configuration ---
# Desired output directory filepath (defaults to %Utility Location%/NPR). Will not create a subfolder.
OUTPUT_DIR_PATH = Path(
    "***INSERT FILEPATH HERE***"
)
ARTICLE_SORT = False  # If True, will sort articles into subfolder based on NPR-provided taxonomy
ARTICLE_SELECTORS = ["nx-s1-", "g-s1-"]
ARTICLE_EXCLUDERS = ["ondemand", "byspotify", "simplecastaudio", "series", "podcasts"]
# --- End Configuration ---
...
def main():
    ...
    try:
        ...
        if not article_links:
            print(
                "🛑 CRITICAL: No article links found using current selector. Please inspect the page source and adjust the 'extract_article_links' function."
            )
            return

        print(f"\n[SUCCESS] Found {len(article_links)} potential article links.")
        ...
        for i, article_link in enumerate(article_links):
            ...
            if article_content:
                if save_markdown_file(
                    article_link, article_content, OUTPUT_DIR_PATH, CURRENT_CONTENT
                ):
                    successful_saves += 1
            else:
                print(f"ERROR: No valid content. Skipping URL.")

        print("\n===========================================================")
        print(
            f"✅ Scraping finished! Successfully saved {successful_saves}/{len(article_links)} articles."
        )
        print("===========================================================")
...
def extract_article_links(html_content: str) -> List[str]:
    """
    Parses the HTML content to find all article links.
    """
    ...
    for link_tag in soup.find_all("a", href=True):
        ...
        for selector in ARTICLE_SELECTORS:
            if selector in article_link:
                # Exclude
                excluded = False
                for excluder in ARTICLE_EXCLUDERS:
                    if excluder in article_link:
                        excluded = True
                if not excluded:
...
def scrape_article_content(url: str) -> Optional[str]:
    ...
    if main_content_area:
        ...
        for tag in main_header_tags:
            ...
            # Determine the tag type and format it accordingly
            tag_name = tag.name
            ...
            if tag_name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                ...
                # Use 'contents' to iterate over everything: both text nodes and tags
                paragraph_segments = []
                parent = tag.parent

                for content in tag.contents:

                    # Skip captions (handled with images in <div> above)
                    ...
                    # If the content is a tag
                        ...
                        #  Disclaimers (no nested formatting support)
...
def save_markdown_file(url: str, content: str, output_dir: Path, html_content: str):
    soup = BeautifulSoup(html_content, "html.parser")

    """Saves the content to a uniquely named Markdown file."""
    ...
    if ARTICLE_SORT == True:
        ...
        file_path = Path(f"{output_dir}/{taxonomy}/{filename}")
    else:
        file_path = Path(f"{output_dir}/{filename}")
    ...
    try:
        # Add Markdown YAML frontmatter header
        ...
        if "N/A" not in article_topics:
            for topic in article_topics:
                if len(topic) <= 25:
                    article_tags.append((sanitize(topic)).replace(" ", "").replace(".", "").replace(",", "").replace("'", ""))
        if "N/A" != article_category:
            if article_category not in article_topics:
                if len(article_category) <= 25:
                    article_tags.append((sanitize(article_category)).replace(" ", "").replace(".", "").replace(",", "").replace("'", ""))
...
def reconstruct(paragraph_segments: list) -> str:
    ...
    if formatted_content:
        # Handle adjacent/double italics edge case
